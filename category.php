...
if(isset($_COOKIE["login_true"])==false){
        session_start();
        $_SESSION["last_url"]="category.php";
	$url = "login.php";
...
function list_category_sel(){
    $.get('category_action.php?action=list_category_sel',
        {r:Math.random(),page_name:'category.php'},
            function(data){
             $('#sel_category').html(data);
             if(last_selected_sel_category!=''){
             	 $('#sel_category').val(last_selected_sel_category);
             }
        });
}

function list_category_sel2(){
    $.get('category_action.php?action=list_category_sel',
        {r:Math.random(),page_name:'category.php'},
            function(data){
             $('#c_id_parent').html(data);
             if(last_selected_sel_category2!=''){
             	 $('#c_id_parent').val(last_selected_sel_category2);
             }
        });
}
...
function get_category(c_id){
        ...
        {r:Math.random(),page_name:'category.php'},
            function(data){
             $('#c_id_parent').html(data);
             $.getJSON('category_action.php',
                    {action:'get_category',c_id:c_id,r:Math.random(),page_name:'category.php'},                    
			function(data){	                        
                  $('#c_id').val(data.c_id);
                  $('#c_id_parent').val(data.c_id_parent);
                  $('#c_sort').val(data.c_sort);
                  $('#c_name').val(data.c_name);
                  $('#c_pic').val(data.c_pic);
                  if(data.c_pic!=null && data.c_pic!=''){
                      $('#pic_span').html('<img src="uploads/'+data.c_pic+'" style="width:'+pic_width+'px;"/>');
                  }
	...
	$(document).ready(function(){
        ...
        function ajaxFileUpload() {			
			...
			if(file!=""){//对格式的判断				
				...
				(
					{
						url: 'uploadAction.php', //用于文件上传的服务器端请求地址
						secureuri: false, //是否需要安全协议，一般设置为false
						fileElementId: 'file1', //文件上传域的ID 也就是<input type="file" id="file1" name="file" />
						dataType: 'json', //返回值类型 
						success: function (data)  //服务器成功响应处理函数
						{
						   $("#pic_span").html("<img src='uploads/"+data.filename+"' style='width:"+pic_width+"px' />");
                                                   $('#c_pic').val(data.filename);
				...
				$("#error").text("Upload file is incorrect");
	...
	function save_category(){
                    ...
                    var c_id =  $('#c_id').val();
            var c_id_parent =  $('#c_id_parent').val();
            var c_sort =  $('#c_sort').val();
            var c_name =  $('#c_name').val();
            var c_pic = $('#c_pic').val();
            
            last_selected_sel_category2 = c_id_parent;
                

		$.post('category_action.php?action=save_category',{
			         c_id : c_id,
             c_id_parent : c_id_parent,
             c_sort : c_sort,
             c_name : c_name,
             c_pic:c_pic,
	...
	function list(page_id){
		 ...
		 var filter = $('#filter').val();     
                 var parent_id = $('#sel_category').val();
                 last_selected_sel_category = parent_id;
		 last_page_id=page_id;
		 $.get('category_action.php',
			{r:Math.random(),
	...
	function list_category(){
	 ...
	 function del_category(d_id){
		 if(confirm('确认要删除吗？')){
 			$.get('category_action.php?action=del_category',
		 	{r:Math.random(),d_id:d_id,page_name:'category.php'},
		 	function(data){
				 if(data=='success'){
	 ...
	 function move_up(cat_id){
	 	 $.get('category_action.php?action=reorder_category',
	 	 {page_name:'category.php',cat_id:cat_id,move_action:'up',r:Math.random()},
	 	 	 function(data){
	 	 	 	 if(data=='success'){
	 	 	 	 	 	go_page(last_page_id);
	 	 	 	 } else {
	 ...
	 function move_down(cat_id){
	 	  $.get('category_action.php?action=reorder_category',
	 	 {page_name:'category.php',cat_id:cat_id,move_action:'down',r:Math.random()},
	 	 	 function(data){
	 	 	 	 if(data=='success'){
	 	 	 	 	 	go_page(last_page_id);
	 	 	 	 }else {
            ...
            <div class="navbar" id='top_div'>
                <div class="navbar-inner">
                  ...
                  <div style="position:inline; float: right;">
                            <ul class="nav">  
                                <li>
                                    <a href='javascript:void(0)' onclick='add_init()'>新增商品分类</a>
                                </li>
                                <li>
                                    <a href='javascript:void(0)' onclick='list_category()'>商品分类列表</a>
                                </li>
                       			<li>
                                    <a href='product_category.php'>分类</a>
                                </li>
                            </ul>
                        </div>	 
	...
	<thead>
		<tr>
			...
			名称
                        ...
                        <input type="hidden" id="c_pic" value="" />
                        </td>                                
                </tr>
                <tr>
			<td style='text-align:right;'>
...
<table id='search_tbl'  class='table table-bordered table-striped table-hover'>
	<tr><td>
		...
		筛选：<input id='filter' name='filter' style='width:150px;' />                                
                &nbsp;
                父分类：<select id="sel_category" style="width:150px;">
